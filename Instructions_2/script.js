...
const folderPanels = document.querySelectorAll('.category-folder__panel');

const DEFAULT_TITLE = 'カテゴリを選択してください';
const DEFAULT_DESCRIPTION = '左側のカテゴリを開いて資料を選択してください。';
...
if (descriptionElement && !descriptionElement.textContent.trim()) {
  descriptionElement.textContent = DEFAULT_DESCRIPTION;
}

function setFolderOpen(panel, open) {
  if (!panel) {
    return;
  }
...
function ensureFolderOpenByButton(button) {
  const panel = button?.closest('.category-folder__panel');
  if (!panel) {
    return;
  }

  if (!panel.open) {
    setFolderOpen(panel, true);
  }
}
...
folderPanels.forEach((panel) => {
  panel.addEventListener('toggle', () => {
    ...
    const activeInside = panel.querySelector('.category.is-active');
    if (activeInside) {
...
function setActiveCategory(activeButton) {
  if (activeButton) {
    ensureFolderOpenByButton(activeButton);
  }
